...
- v39 keeps v38 speed/counting and replaces nearest-frontier exploration with an online topological gateway policy.
...
class OccupancyGridMapper:
    ...
    def regularized_occupied_cells(self) -> set[Cell]:
        ...
        raw_occ = self._raw_occupied_cells()
        if not raw_occ:
            return set()
        ...
        if bool(getattr(self, "map_regularize_keep_strong_raw", True)):
            ...
            out_cells.update(self._raw_occupied_cells(threshold=strong_threshold))
...
class FrontierPlanner:
    ...
    def plan_to_best_frontier(
        ...
        for goal in candidates[:max_frontiers_to_try]:
            ...
            score = len(path) + turn_penalty + revisit_penalty + recent_penalty + turnback_penalty + gateway_penalty - unknown_reward - progress_reward
            if score < best_score:
                best_score = score
                best_path = path
...
class QualifierMission:
    def __init__(self, args: argparse.Namespace):
        ...
        for _name in (
            ...
            "frontier_unknown_weight", "frontier_unknown_ahead_weight", "frontier_progress_weight",
    ...
    async def drive_express_corridor_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray], reason: str = "express") -> bool:
        ...
        if await self.maybe_take_side_opening(pose, left, center, right, reason="express"):
            return True
    ...
    async def scan_viable_pathways(self, reason: str = "local_scan") -> Optional[float]:
        ...
        if bool(getattr(self.args, "topological_gateway_mode", True)) and not reason.startswith("legacy_"):
        ...
        for y in headings:
            ...
            print(f"[CENTERLINE] yaw={y:.0f} L={left:.2f} C={center:.2f} R={right:.2f} unknown={unknown_gain} visited={visit_penalty:.1f} backtrack={backtrack_blocked} viable={is_viable} score={score:.2f}")
    ...
    async def drive_centerline_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray], reason: str = "centerline") -> bool:
        ...
        if await self.maybe_take_side_opening(pose, left, center, right, reason=reason):
            return True
    ...
    async def drive_path_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray]) -> None:
        ...
        if not self.mapper.path_segment_collision_free(
    ...
    async def run(self) -> None:
        ...
        # v24: fast startup.  In a 5-minute qualifier, scanning 14 headings before
        # moving is too expensive.  Default is a quick centreline scan over only a
        # few headings, then commit to the best corridor while continuing to map in
        # motion.  Use --startup-scan-mode full if you want the slower v23 behaviour.
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Topological gateway exploration qualifier mission with YOLO barrel counting")
    ...
    p.add_argument("--output-dir", default="qualifier_output_v39")
    ...
    p.add_argument("--cruise-speed-m-s", type=float, default=1.05)
    ...
    p.add_argument("--map-regularize-keep-strong-raw", action=argparse.BooleanOptionalAction, default=True, help="Also keep very strong raw occupied cells so partial box faces are not deleted")
    ...
    p.add_argument("--frontier-unknown-weight", type=float, default=0.16, help="v38: reward for frontiers bordering large unknown regions")
    p.add_argument("--frontier-unknown-ahead-weight", type=float, default=0.25, help="v38: reward for unknown space beyond the frontier direction")
    p.add_argument("--frontier-progress-weight", type=float, default=0.020, help="v38: small reward for choosing farther exits over nearby perimeter edges")
    ...
    p.add_argument("--topo-frontier-gateway-bonus", type=float, default=10.0)
    p.add_argument("--topo-frontier-depth-bonus", type=float, default=0.08)
    p.add_argument("--topo-frontier-perimeter-penalty", type=float, default=30.0)
    ...
    p.add_argument("--topo-normal-scan-max-headings", type=int, default=5)
    p.add_argument("--topo-gateway-scan-max-headings", type=int, default=8)
    p.add_argument("--topo-front-block-max-headings", type=int, default=7)
    ...
    p.add_argument("--topo-side-check-gap-s", type=float, default=1.2)
    p.add_argument("--waypoint-radius-m", type=float, default=1.55)
    p.add_argument("--path-stride-m", type=float, default=2.35, help="Distance between simplified A* waypoints on straight segments. Corners keep extra waypoints.")
    p.add_argument("--path-turn-padding-cells", type=int, default=1, help="Keep this many extra A* cells around a turn so the drone does not cut/collide with corners")
    ...
    p.add_argument("--startup-scan-mode", choices=["quick", "full", "none"], default="quick", help="quick=scan only a few headings and move; full=v23-style full scan; none=move immediately")
    ...
    p.add_argument("--frontier-scan-max-headings", type=int, default=3, help="Limit expensive frontier yaw scans")
    ...
    p.add_argument("--local-corridor-speed-m-s", type=float, default=0.78)
    ...
    p.add_argument("--express-corridor-speed-m-s", type=float, default=1.20, help="Target speed inside a detected straight corridor; capped by --cruise-speed-m-s")
    ...
    p.add_argument("--ghost-clear-center-m", type=float, default=2.6)
    p.add_argument("--ghost-clear-min-side-m", type=float, default=0.75)
    p.add_argument("--ghost-clear-radius-m", type=float, default=0.75)
    p.add_argument("--ghost-clear-amount", type=int, default=7)
    p.add_argument("--ghost-verify-max-turn-deg", type=float, default=75.0)
    ...
    p.add_argument("--zone-loop-min-travel-m", type=float, default=24.0)
    p.add_argument("--zone-loop-max-span-m", type=float, default=16.0)
    p.add_argument("--zone-loop-close-m", type=float, default=2.0)
    p.add_argument("--zone-loop-min-current-visit", type=int, default=5)
    p.add_argument("--zone-loop-cooldown-s", type=float, default=18.0)
    ...
    # YOLO / counting.
    p.add_argument("--detect-interval-s", type=float, default=0.55, help="Run YOLO at this interval in count mode")
