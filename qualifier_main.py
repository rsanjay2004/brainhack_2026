...
RRT_MAX_ITER = 2000   # iterations per plan (500 was too few for dense pillar fields)
...
# Avoidance
SAFE_DIST = 2.0   # was 3.0 — too large, avoidance permanently active in indoor arena
CRIT_DIST = 1.5
...
STUCK_TIMEOUT_S = 7.0
STUCK_DIST_M = 0.4
WP_APPROACH_TIMEOUT = 25.0   # s — force-advance WP if not arrived within this time
STUCK_ESCAPE_M = 2.5

# Detection gate — set true to enable barrel detection during navigation debugging
...
class QualifierMission:
    def __init__(self, model_path=""):
        ...
        self._last_min_clearance = SAFE_DIST  # updated each velocity tick
    ...
    def _check_stuck(self):
        ...
        if getattr(self, '_last_min_clearance', SAFE_DIST) < CRIT_DIST:
            ...
            if time.monotonic() - self._avoidance_since < 8.0:
                self._stuck_timer = time.monotonic()
                return False
    ...
    def _compute_velocity_setpoint(self, pose, target_n, target_e, target_d, depth):
        ...
        if depth is not None:
            ...
            if blocked:
                ...
                if self._avoid_log_tick % 120 == 1:  # ~0.25 Hz — reduce spam
                    cl = info["clearance"]
                    print(f"[AVOID] T={self._elapsed():.0f}s  "
        ...
        if all_critical:
            # All depth sectors blocked — find the most open direction in GlobalMapper
            # instead of using memory repulsion (which can point randomly when surrounded).
            open_n, open_e = self._find_open_escape_direction(
                cur_n, cur_e, goal_n, goal_e
            )
            if self._avoid_log_tick % 120 == 1:
                open_deg = math.degrees(math.atan2(open_e, open_n))
                goal_deg = math.degrees(math.atan2(goal_e, goal_n))
                print(f"[AVOID] T={self._elapsed():.0f}s ALL_CRIT "
                      f"pos=({cur_n:.1f},{cur_e:.1f}) "
                      f"escape_dir={open_deg:.0f}° goal_dir={goal_deg:.0f}°")
            blend_n = open_n + W_WALL * wall_n
            blend_e = open_e + W_WALL * wall_e
        elif blocked:
    ...
    async def _tick_explore(self):
        ...
        # Rate-limited NAV status log (every 10s)
        ...
        if _nav_now - self._nav_log_time >= 10.0:
            ...
            print(f"[NAV] T={self._elapsed():.0f}s  "
                  f"pos=({cur_n:.1f},{cur_e:.1f}) alt={-pose['down']:.1f}m yaw={pose['yaw_deg']:.0f}° "
                  f"WP {self._wp_idx}/{len(self._waypoints)} "
                  f"→ N={wp[0]:.1f} E={wp[1]:.1f} dist={horiz:.1f}m "
                  f"t_wp={approach_elapsed:.0f}s clr={getattr(self, '_last_min_clearance', 9.9):.1f}m "
                  f"spd={spd:.2f}m/s hdg={hdg:.0f}° "
                  f"obs5m={obs_count_5m} map={obs_near.shape[0]}pts")
