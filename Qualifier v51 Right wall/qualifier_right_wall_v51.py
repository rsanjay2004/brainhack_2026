#!/usr/bin/env python3
"""
qualifier_right_wall_v49.py

Right-wall-following autonomous navigation for the drone barrel-counting qualifier.

This version is based on qualifier_right_wall_v51.py. It keeps the existing YOLO red/yellow
barrel counter, RGB/depth subscribers, occupancy grid mapper, debug map saving, and
self-clearing occupancy bubble, but replaces the default navigation policy with a
right-wall follower.
...
- v49 keeps the working v48 mapping/counting but fixes the dead-end escape contradiction where an escape route that intentionally reused recent cells was immediately rejected by the no-backtracking gate. It adds a recent-path escape lock so the drone decisively follows the route out of a dead end/local loop before resuming normal exploration.
...
def resolve_yolo_model_path(args: argparse.Namespace, output_dir: Path) -> str:
    ...
    if getattr(args, "model", ""):
        ...
        elif str(args.model) == "models/barrel_best.pt":
            print(f"[YOLO] Default model {model} not found; falling back to automatic local weight search")
            args.model = ""
        else:
...
class QualifierMission:
    def __init__(self, args: argparse.Namespace):
        ...
        # Right-wall navigation state.  This mode deliberately does not use the
        # frontier/topological planner.  Mapping, photo/YOLO detection, safety,
        # and self-footprint clearing still run exactly as normal.
        self.right_wall_no_wall_count = 0
        self.right_wall_front_block_count = 0
        self.last_right_wall_log_ms = 0
        self.right_wall_last_action = "init"
        # Staged right-corner turn state.  A simple yaw-right + right-drift
        # command can cut the inside corner and hit the wall.  The staged corner
        # controller first carries the drone slightly past the corner, then arcs
        # right while biasing away from the inside wall until the new right wall
        # is reacquired.
        self.right_wall_corner_active = False
        self.right_wall_corner_start_pose: Optional[Dict[str, float]] = None
        self.right_wall_corner_start_yaw = 0.0
        self.right_wall_corner_target_yaw = 0.0
        self.right_wall_corner_start_ms = 0
        self.right_wall_corner_last_log_ms = 0
        ...
        # v46 verified-open corridor bypass. If the occupancy grid says a route is
        # blocked but a settled depth look down that exact route is clear, the
        # blockage is a map-only artefact.  Do not immediately replan into the same
        # blocked segment again; allow the drone to commit through the verified-open
        # corridor for a short distance/time.
        self.ghost_bypass_until_ms = 0
        self.ghost_bypass_start_pose: Optional[Dict[str, float]] = None
        self.ghost_bypass_target: Optional[Tuple[float, float]] = None
        self.ghost_bypass_yaw: Optional[float] = None

        # v49 recent-path escape lock.  If the strict non-backtracking planner
        # fails, v48 correctly computes an escape route that reuses recent cells
        # to leave a dead-end/local loop.  The old avoid-recent-path gate could
        # immediately invalidate that same route, causing an endless
        # escape-fallback -> refuse-recent -> centerline loop.  These fields mark
        # a computed escape route as intentionally allowed to reuse recent cells
        # until the drone has travelled a useful distance or the timeout expires.
        self.recent_escape_until_ms = 0
        self.recent_escape_start_pose: Optional[Dict[str, float]] = None
        self.recent_escape_min_m = 0.0
        self.current_path_allows_recent = False
        self.current_path_reason = ""
        self.recent_escape_count = 0

        # v47 open-room gateway crossing.  In big rooms with box obstacles, the
        # global frontier route can orbit the perimeter or avoid narrow gaps too
        # conservatively.  This state lets the drone commit through a currently
        # visible, depth-clear interior corridor toward unknown space instead of
        # circling around boxes.
        self.open_room_drive_yaw: Optional[float] = None
        self.open_room_drive_until_ms = 0
        self.open_room_drive_start_pose: Optional[Dict[str, float]] = None
        self.open_room_last_select_ms = 0
    ...
    async def scan_topological_gateways(self, reason: str = "topo_gateway") -> Optional[float]:
        ...
        if "dead_end" in reason or "backtrack" in reason:
            # v48: at a confirmed/front-blocked corridor end, the correct way out is
            # often the recently travelled corridor behind the drone.  Check the U-turn
            # direction first and allow recent-path reuse for this explicit escape mode.
            deltas = [180.0, 135.0, -135.0, 90.0, -90.0, 45.0, -45.0, 0.0]
            max_headings = int(getattr(self.args, "topo_dead_end_scan_max_headings", 6))
        ...
        for y in headings:
            ...
            # v48: normal gateway selection needs unknown space ahead.  Explicit
            # dead-end/backtrack escape is different: the route out can be the already
            # travelled corridor, so require clearance rather than unknown cells.
            if "dead_end" in reason or "backtrack" in reason:
                viable = (
                    center >= float(getattr(self.args, "topo_dead_end_escape_min_center_m", 1.75))
                    and side_balance >= float(getattr(self.args, "topo_dead_end_escape_min_side_m", 0.35))
                    and not blacklisted
                )
            else:
                viable = (
    ...
    def start_ghost_bypass(self, pose: Dict[str, float], target: Tuple[float, float], yaw_deg: float) -> None:
        """Temporarily trust a depth-verified open corridor over stale map cells."""
        self.ghost_bypass_until_ms = now_ms() + int(1000.0 * float(getattr(self.args, "ghost_bypass_after_clear_s", 5.0)))
        self.ghost_bypass_start_pose = dict(pose)
        self.ghost_bypass_target = (float(target[0]), float(target[1]))
        self.ghost_bypass_yaw = float(yaw_deg)
        print(
            f"[GHOST] verified open corridor; bypassing map-only blockage "
            f"for {float(getattr(self.args, 'ghost_bypass_after_clear_s', 5.0)):.1f}s "
            f"or {float(getattr(self.args, 'ghost_bypass_min_m', 2.6)):.1f}m"
        )

    def ghost_bypass_active_for(self, pose: Dict[str, float], target: Tuple[float, float]) -> bool:
        """True when the current blocked first segment matches a recently verified-open route."""
        if now_ms() > int(self.ghost_bypass_until_ms):
            return False
        if self.ghost_bypass_start_pose is None or self.ghost_bypass_target is None or self.ghost_bypass_yaw is None:
            return False
        travelled = math.hypot(
            float(pose["north"]) - float(self.ghost_bypass_start_pose["north"]),
            float(pose["east"]) - float(self.ghost_bypass_start_pose["east"]),
        )
        if travelled > float(getattr(self.args, "ghost_bypass_min_m", 2.6)):
            return False
        target_shift = math.hypot(float(target[0]) - self.ghost_bypass_target[0], float(target[1]) - self.ghost_bypass_target[1])
        if target_shift > float(getattr(self.args, "ghost_bypass_target_tolerance_m", 2.2)):
            return False
        yaw_to_target = yaw_from_vector_deg(float(target[0]) - float(pose["north"]), float(target[1]) - float(pose["east"]))
        if abs(yaw_error_deg(yaw_to_target, float(self.ghost_bypass_yaw))) > float(getattr(self.args, "ghost_bypass_yaw_tolerance_deg", 35.0)):
            return False
        return True
    ...
    async def handle_front_blocked(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray], reason: str = "front_block") -> None:
        ...
        # v48: if no fresh side branch is found at a corridor end, do not fall into
        # repeated recovery scans.  First try a permissive U-turn/escape heading that
        # is allowed to reuse the recent corridor, because that is exactly how to
        # leave a true dead end.  If that also fails, directly follow breadcrumbs.
        if bool(getattr(self.args, "dead_end_escape_on_front_block_fail", True)):
            yaw_escape = await self.scan_viable_pathways(reason="dead_end_backtrack_scan")
            if yaw_escape is not None:
                p = self.pose() or pose
                d = self.depth.get_frame() if self.depth.get_frame() is not None else depth_frame
                print(f"[DEAD_END] permissive corridor escape selected yaw={yaw_escape:.0f}")
                await self.drive_centerline_step(p, d, reason="dead_end_backtrack_scan")
                self.front_block_count = 0
                return

        # Only after a side-branch scan fails should this be treated as a true dead
        # end requiring breadcrumb backtracking.  v48 allows this even if a side
        # sector looks moderately open; a failed branch scan is stronger evidence
        # than the raw L/C/R thresholds.
        if bool(getattr(self.args, "dead_end_force_breadcrumb_after_failed_bypass", True)) or self.is_dead_end_view(left, center, right, depth_frame):
    ...
    async def scan_viable_pathways(self, reason: str = "local_scan") -> Optional[float]:
        ...
        if "dead_end" in reason or "backtrack" in reason:
            # v48: escape a true dead end by checking the path back out first.
            deltas = [180.0, 3.0 * step, -3.0 * step, 2.0 * step, -2.0 * step, step, -step, 0.0]
            max_headings = int(getattr(self.args, "dead_end_scan_max_headings", 6))
        elif reason == "startup":
    ...
    def start_recent_escape_lock(self, pose: Dict[str, float], reason: str = "escape_fallback") -> None:
        """Allow the current planned route to reuse recent cells intentionally.

        This is only for escape routes produced after strict non-backtracking
        planning failed.  It prevents the route from being cancelled by the
        avoid-recent-path filter before the drone has actually exited the dead
        end/local loop.
        """
        self.current_path_allows_recent = True
        self.current_path_reason = reason
        self.recent_escape_start_pose = dict(pose)
        self.recent_escape_min_m = float(getattr(self.args, "recent_escape_commit_min_m", 5.5))
        self.recent_escape_until_ms = now_ms() + int(float(getattr(self.args, "recent_escape_commit_s", 16.0)) * 1000)
        self.recent_escape_count += 1
        yaw = pose.get("yaw_deg", 0.0)
        if self.current_path:
            first = self.current_path[0]
            yaw = yaw_from_vector_deg(first[0] - pose["north"], first[1] - pose["east"])
        self.start_progress_commitment(
            pose,
            yaw,
            f"recent_escape:{reason}",
            duration_s=float(getattr(self.args, "recent_escape_commit_s", 16.0)),
            min_dist_m=float(getattr(self.args, "recent_escape_commit_min_m", 5.5)),
        )
        print(
            f"[ESCAPE] recent-path reuse allowed reason={reason} "
            f"min_m={self.recent_escape_min_m:.1f} time_s={float(getattr(self.args, 'recent_escape_commit_s', 16.0)):.1f}"
        )

    def recent_escape_active(self, pose: Optional[Dict[str, float]] = None) -> bool:
        if not self.current_path_allows_recent:
            return False
        if now_ms() > self.recent_escape_until_ms:
            self.current_path_allows_recent = False
            self.current_path_reason = ""
            return False
        if pose is not None and self.recent_escape_start_pose is not None:
            travelled = math.hypot(
                pose["north"] - self.recent_escape_start_pose["north"],
                pose["east"] - self.recent_escape_start_pose["east"],
            )
            if travelled >= self.recent_escape_min_m:
                self.current_path_allows_recent = False
                self.current_path_reason = ""
                print(f"[ESCAPE] recent-path reuse lock complete travelled={travelled:.1f}m")
                return False
        return True

    def clear_recent_escape_lock(self) -> None:
        self.current_path_allows_recent = False
        self.current_path_reason = ""
        self.recent_escape_until_ms = 0
        self.recent_escape_start_pose = None
        self.recent_escape_min_m = 0.0
    ...
    def replan(self, pose: Dict[str, float]) -> None:
        ...
        escape_path = False
        if path is None and (
            bool(getattr(self.args, "allow_recent_path_fallback", False))
            or bool(getattr(self.args, "escape_frontier_fallback", True))
        ):
            # v48: if every reachable frontier requires crossing a few recently
            # travelled cells, allow a one-shot escape route to a high-information
            # frontier instead of circling locally.  This is not ordinary backtracking;
            # it is only used after the strict non-backtracking planner failed.
            path = self.planner.plan_to_best_frontier(
                pose,
                min_frontier_dist_m=self.args.min_frontier_dist_m,
                max_frontiers_to_try=self.args.max_frontiers_to_try,
                ...
                recent_cells=recent_cells,
                avoid_recent=False,
                recent_penalty_weight=float(getattr(self.args, "escape_recent_path_penalty", 2.0)),
                turnback_penalty_weight=float(getattr(self.args, "turnback_penalty", 5.0)),
            )
            if path:
                escape_path = True
                print("[PLAN] escape fallback reused recent cells to reach an unexplored frontier")
        ...
        if path:
            ...
            goal = path[-1]
            if escape_path:
                self.start_recent_escape_lock(pose, reason="escape_frontier_fallback")
            else:
                self.clear_recent_escape_lock()
                if path:
                    first = path[0]
            ...
            print(f"[PLAN] path_len={len(path)} goal N={goal[0]:.1f} E={goal[1]:.1f} known_cells={len(self.mapper.logodds)} rejected={len(rejected)} recent_cells={len(recent_cells)} escape={int(escape_path)}")
        else:
            self.current_path = []
            self.clear_recent_escape_lock()
    ...
    def corridor_centering_view(self, left: float, center: float, right: float) -> bool:
        """Return True when current depth sectors look like a corridor/doorway entry.

        v45: The drone was reaching the upper corridor but not aligning to the
        corridor centre before entering.  This lightweight detector is deliberately
        local and conservative: it activates only when there is useful forward
        clearance and at least one nearby side boundary.  It is used to bias
        global-path motion toward the centreline; it does not replace emergency
        avoidance.
        """
        if not bool(getattr(self.args, "global_corridor_centering", True)):
            return False
        if center < float(getattr(self.args, "corridor_centering_min_center_m", 2.4)):
            return False
        min_side = min(left, right)
        max_side = max(left, right)
        min_valid = float(getattr(self.args, "corridor_centering_min_valid_side_m", 0.25))
        side_wall_max = float(getattr(self.args, "corridor_centering_side_wall_max_m", 3.8))
        open_side_max = float(getattr(self.args, "corridor_centering_open_side_max_m", 6.0))
        one_side_close = min_side <= float(getattr(self.args, "corridor_centering_one_side_trigger_m", 1.65)) and max_side <= open_side_max
        two_walls_visible = min_side >= min_valid and left <= side_wall_max and right <= side_wall_max
        return bool(two_walls_visible or one_side_close)

    def apply_global_corridor_centering(
        self,
        pose: Dict[str, float],
        desired_vn: float,
        desired_ve: float,
        ...
        """Bias a global/A* command toward the local corridor centreline.

        This fixes a common failure in narrow corridors: the global path is
        geometrically valid but runs too close to one wall/corner.  The drone then
        clips the corridor edge or decides that the path is blocked.  We keep the
        forward component of the global command, but add a bounded body-frame
        lateral correction away from the closer wall.
        """
        info: Dict[str, float] = {"active": 0.0, "correction": 0.0, "forward": 0.0, "right_body": 0.0}
        if not self.corridor_centering_view(left, center, right):
            return desired_vn, desired_ve, info

        forward_body, right_body = ned_to_body(desired_vn, desired_ve, pose["yaw_deg"])
        if forward_body <= 0.02:
            return desired_vn, desired_ve, info

        # Positive error means there is more space on the right, so move right.
        clearance_error = right - left
        deadband = float(getattr(self.args, "corridor_centering_balance_deadband_m", 0.20))
        if abs(clearance_error) <= deadband and min(left, right) > float(getattr(self.args, "corridor_centering_close_side_m", 0.85)):
            info.update({"active": 1.0, "correction": 0.0, "forward": forward_body, "right_body": right_body})
            return desired_vn, desired_ve, info

        gain = float(getattr(self.args, "corridor_centering_gain", 0.22))
        max_lat = float(getattr(self.args, "corridor_centering_max_lateral_m_s", 0.22))
        correction = clamp(gain * clearance_error, -max_lat, max_lat)

        close_side = float(getattr(self.args, "corridor_centering_close_side_m", 0.85))
        if left < close_side:
            correction = max(correction, min(max_lat, 0.18))
        if right < close_side:
            correction = min(correction, -min(max_lat, 0.18))

        right_body = clamp(right_body + correction, -max_lat, max_lat)

        # If badly off-centre, slow forward motion slightly until centred.  This is
        # still much faster than stopping to rescan, but it prevents corner clipping.
        if abs(clearance_error) > float(getattr(self.args, "corridor_centering_slow_imbalance_m", 1.15)):
            forward_body = min(forward_body, float(getattr(self.args, "corridor_centering_entry_speed_m_s", 0.55)))
        if min(left, right) < close_side:
            forward_body = min(forward_body, float(getattr(self.args, "corridor_centering_close_speed_m_s", 0.42)))

        vn, ve = body_to_ned(forward_body, right_body, pose["yaw_deg"])
        info.update({"active": 1.0, "correction": correction, "forward": forward_body, "right_body": right_body})
        if now_ms() - getattr(self, "last_corridor_centering_log_ms", 0) > 800:
            self.last_corridor_centering_log_ms = now_ms()
            print(
                f"[CENTERING] corridor L={left:.2f} C={center:.2f} R={right:.2f} "
                f"err={clearance_error:.2f} corr={correction:.2f} body=({forward_body:.2f},{right_body:.2f})"
            )
        return vn, ve, info

    async def drive_path_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray]) -> None:
        """Dispatch navigation according to --navigation-mode.

        The default in this right-wall file is --navigation-mode right_wall.  The
        original v49 frontier/global-path navigator is retained under
        --navigation-mode frontier for comparison/debugging.
        """
        nav_mode = str(getattr(self.args, "navigation_mode", "right_wall")).lower()
        if nav_mode == "right_wall":
            await self.drive_right_wall_step(pose, depth_frame)
        else:
            await self.drive_path_step_frontier(pose, depth_frame)

    async def drive_right_wall_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray]) -> None:
        """Right-hand wall follower with occupancy mapping and YOLO left intact.

        Rule order:
        1. If a right corner is active, execute a staged wide-entry arc.
        2. If the right side opens, start that staged corner turn.
        3. Else if the front is blocked, turn left.
        4. Else drive forward while holding a target clearance to the right wall.

        Translation still goes through DepthSafetyFilter before it is sent to the
        drone.  The occupancy mapper is updated in run() before this method and
        the self-clearing bubble is refreshed here immediately before navigation.
        """
        left, center, right = self.clearances(depth_frame)
        self.mapper.clear_pose_footprint(pose, reason="right_wall_pre_navigation")
        self.update_travel_history(pose)

        if depth_frame is None:
            print("[RIGHT_WALL] no depth frame; holding position")
            await self.stop_motion(0.2)
            return

        now = now_ms()
        if now - getattr(self, "last_right_wall_log_ms", 0) > int(1000.0 * float(getattr(self.args, "right_wall_log_interval_s", 0.5))):
            self.last_right_wall_log_ms = now
            print(
                f"[RIGHT_WALL] N={pose['north']:.1f} E={pose['east']:.1f} yaw={pose['yaw_deg']:.0f} "
                f"L={left:.2f} C={center:.2f} R={right:.2f} saved={self.logger.saved_count}"
            )

        target_right = float(getattr(self.args, "right_wall_target_m", 1.30))
        open_m = float(getattr(self.args, "right_wall_open_m", 2.40))
        front_turn_m = float(getattr(self.args, "right_wall_front_turn_m", 1.55))
        corner_front_m = float(getattr(self.args, "right_wall_corner_min_front_m", 0.85))
        right_too_close_m = float(getattr(self.args, "right_wall_too_close_m", 0.65))
        right_turn_deg = float(getattr(self.args, "right_wall_right_turn_deg", 32.0))
        left_turn_deg = float(getattr(self.args, "right_wall_left_turn_deg", 36.0))
        max_lat = float(getattr(self.args, "right_wall_max_lateral_m_s", 0.22))
        lateral_gain = float(getattr(self.args, "right_wall_lateral_gain", 0.32))
        yaw_gain_deg_per_m = float(getattr(self.args, "right_wall_yaw_gain_deg_per_m", 10.0))
        yaw_bias_max = float(getattr(self.args, "right_wall_max_yaw_bias_deg", 14.0))

        if getattr(self, "right_wall_corner_active", False):
            handled = await self.drive_right_wall_corner_step(pose, depth_frame, left, center, right)
            if handled:
                return

        front_supported = bool(self.safety.front_obstacle_supported(depth_frame, float(getattr(self.args, "emergency_stop_m", 1.15))))
        front_blocked = center <= front_turn_m or (center <= float(getattr(self.args, "emergency_stop_m", 1.15)) and front_supported)
        right_open = right >= open_m

        if right_open:
            self.right_wall_no_wall_count += 1
        else:
            self.right_wall_no_wall_count = 0

        # Right-hand rule: when the right side opens, turn into it, but do not
        # cut the inside corner.  The previous direct yaw-right + right-drift
        # command could clip the corner.  Start a staged wide-entry turn instead.
        right_open_confirm = int(getattr(self.args, "right_wall_open_confirm_frames", 2))
        if right_open and self.right_wall_no_wall_count >= max(1, right_open_confirm) and center >= corner_front_m:
            self.start_right_wall_corner(pose, reason="right_opening")
            await self.drive_right_wall_corner_step(pose, depth_frame, left, center, right)
            return

        # If the path ahead is blocked and there is no right-side opening, turn
        # left until the center sector clears.  This keeps the wall on the drone's
        # right as it follows the perimeter.
        if front_blocked:
            self.right_wall_front_block_count += 1
            target_yaw = wrap_deg(float(pose["yaw_deg"]) - left_turn_deg)
            await self.drone.send_velocity(0.0, 0.0, 0.0, target_yaw)
            self.right_wall_last_action = "turn_left_front_blocked"
            if now - getattr(self, "last_right_wall_front_log_ms", 0) > 700:
                self.last_right_wall_front_log_ms = now
                print(f"[RIGHT_WALL] front blocked C={center:.2f} supported={front_supported}; turning left to yaw={target_yaw:.0f}")
            self.record_waypoint(pose, event="right_wall_turn_left_front_blocked", target_yaw=target_yaw, clearances=(left, center, right), force=True)
            return
        self.right_wall_front_block_count = 0

        # Normal wall-following: move forward and use lateral velocity + small yaw
        # bias to hold a fixed right-wall clearance.
        clearance_error = right - target_right
        right_cmd = clamp(lateral_gain * clearance_error, -max_lat, max_lat)
        forward = min(float(getattr(self.args, "right_wall_speed_m_s", self.args.cruise_speed_m_s)), float(self.args.cruise_speed_m_s))

        if center < float(getattr(self.args, "right_wall_slow_front_m", self.args.front_slow_m)):
            forward = min(forward, float(self.args.slow_speed_m_s))
        if right < right_too_close_m:
            right_cmd = -max_lat
            forward = min(forward, float(getattr(self.args, "right_wall_close_speed_m_s", self.args.slow_speed_m_s)))
        elif right > open_m:
            # This is mostly handled by the right-turn branch above.  Keep a gentle
            # right drift for the single frame before the confirm threshold is met.
            right_cmd = max(right_cmd, min(max_lat, float(getattr(self.args, "right_wall_search_lateral_m_s", 0.12))))

        yaw_bias = clamp(yaw_gain_deg_per_m * clearance_error, -yaw_bias_max, yaw_bias_max)
        target_yaw = wrap_deg(float(pose["yaw_deg"]) + yaw_bias)

        desired_vn, desired_ve = body_to_ned(forward, right_cmd, float(pose["yaw_deg"]))
        safe_vn, safe_ve, avoid_info = self.safety.filter_velocity_ned(desired_vn, desired_ve, pose, depth_frame)

        if avoid_info.get("active") and now - getattr(self, "last_right_wall_avoid_log_ms", 0) > 600:
            self.last_right_wall_avoid_log_ms = now
            print(
                f"[RIGHT_WALL_AVOID] {avoid_info.get('reason')} desired=({desired_vn:.2f},{desired_ve:.2f}) "
                f"safe=({safe_vn:.2f},{safe_ve:.2f}) L={avoid_info.get('left',0):.2f} "
                f"C={avoid_info.get('center',0):.2f} R={avoid_info.get('right',0):.2f} "
                f"corridor={avoid_info.get('corridor',0):.2f} count={avoid_info.get('corridor_count',0)}"
            )

        if avoid_info.get("emergency"):
            reason = str(avoid_info.get("reason", ""))
            if "front" in reason or center < float(getattr(self.args, "emergency_stop_m", 1.15)):
                target_yaw = wrap_deg(float(pose["yaw_deg"]) - left_turn_deg)
                await self.drone.send_velocity(0.0, 0.0, 0.0, target_yaw)
                self.record_waypoint(pose, event="right_wall_emergency_turn_left", target_yaw=target_yaw, clearances=(left, center, right), force=True)
                return
            await self.stop_motion(0.05)
            self.record_waypoint(pose, event="right_wall_emergency_hold", target_yaw=pose["yaw_deg"], clearances=(left, center, right), force=True)
            return

        if math.hypot(safe_vn, safe_ve) < float(getattr(self.args, "min_progress_speed_m_s", 0.06)):
            # The safety layer can crawl to zero near projection artefacts.  Hold
            # yaw briefly and let the next depth frame decide instead of invoking
            # the old frontier recovery stack.
            await self.stop_motion(0.05)
            self.record_waypoint(pose, event="right_wall_safety_hold", target_yaw=target_yaw, clearances=(left, center, right), force=False)
            return

        await self.drone.send_velocity(safe_vn, safe_ve, 0.0, target_yaw)
        self.right_wall_last_action = "follow_right_wall"
        self.record_waypoint(pose, event="right_wall_follow", target_yaw=target_yaw, clearances=(left, center, right), force=False)

    def start_right_wall_corner(self, pose: Dict[str, float], reason: str = "right_opening") -> None:
        """Start a staged right turn that avoids clipping the inside corner."""
        corner_deg = float(getattr(self.args, "right_wall_corner_turn_deg", 90.0))
        self.right_wall_corner_active = True
        self.right_wall_corner_start_pose = pose.copy()
        self.right_wall_corner_start_yaw = float(pose["yaw_deg"])
        self.right_wall_corner_target_yaw = wrap_deg(float(pose["yaw_deg"]) + corner_deg)
        self.right_wall_corner_start_ms = now_ms()
        self.right_wall_corner_last_log_ms = 0
        self.right_wall_no_wall_count = 0
        self.right_wall_front_block_count = 0
        print(
            f"[RIGHT_CORNER] start reason={reason} "
            f"yaw={pose['yaw_deg']:.0f}->{self.right_wall_corner_target_yaw:.0f}"
        )
        self.record_waypoint(
            pose,
            event=f"right_wall_corner_start:{reason}",
            target_yaw=self.right_wall_corner_target_yaw,
            force=True,
        )

    def right_wall_corner_travelled_m(self, pose: Dict[str, float]) -> float:
        if self.right_wall_corner_start_pose is None:
            return 0.0
        return math.hypot(
            float(pose["north"]) - float(self.right_wall_corner_start_pose["north"]),
            float(pose["east"]) - float(self.right_wall_corner_start_pose["east"]),
        )

    def stop_right_wall_corner(self, pose: Optional[Dict[str, float]], reason: str) -> None:
        if getattr(self, "right_wall_corner_active", False):
            yaw = float(getattr(self, "right_wall_corner_target_yaw", 0.0))
            print(f"[RIGHT_CORNER] stop reason={reason} target_yaw={yaw:.0f}")
            self.record_waypoint(pose, event=f"right_wall_corner_stop:{reason}", target_yaw=yaw, force=True)
        self.right_wall_corner_active = False
        self.right_wall_corner_start_pose = None
        self.right_wall_corner_start_ms = 0

    async def drive_right_wall_corner_step(
        self,
        pose: Dict[str, float],
        depth_frame: Optional[np.ndarray],
        left: float,
        ...
        """Execute a wide-entry right corner instead of cutting the inside wall.

        The old right-open behaviour immediately commanded yaw-right plus lateral
        motion to body-right.  That is exactly the geometry that clips the inside
        corner.  This controller splits the turn into:

        1. entry: move slightly forward while biasing left/away from the inside;
        2. arc: yaw toward the new corridor with slow forward motion and no
           aggressive right drift;
        3. reacquire: finish once the new right wall is visible at a safe distance.
        """
        if depth_frame is None:
            self.stop_right_wall_corner(pose, "no_depth")
            return False

        now = now_ms()
        target_right = float(getattr(self.args, "right_wall_target_m", 1.30))
        too_close = float(getattr(self.args, "right_wall_too_close_m", 0.70))
        target_yaw = float(getattr(self, "right_wall_corner_target_yaw", wrap_deg(pose["yaw_deg"] + 90.0)))
        start_yaw = float(getattr(self, "right_wall_corner_start_yaw", pose["yaw_deg"]))
        corner_deg = max(1.0, abs(yaw_error_deg(target_yaw, start_yaw)))
        yaw_done = clamp(abs(yaw_error_deg(pose["yaw_deg"], start_yaw)) / corner_deg, 0.0, 1.0)
        travelled = self.right_wall_corner_travelled_m(pose)
        elapsed_s = max(0.0, (now - int(getattr(self, "right_wall_corner_start_ms", now))) / 1000.0)

        timeout_s = float(getattr(self.args, "right_wall_corner_timeout_s", 5.5))
        if elapsed_s > timeout_s:
            self.stop_right_wall_corner(pose, "timeout")
            return False

        reacquire_m = float(getattr(self.args, "right_wall_corner_reacquire_m", 2.05))
        yaw_tol = float(getattr(self.args, "right_wall_corner_finish_yaw_tol_deg", 16.0))
        finish_front_m = float(getattr(self.args, "right_wall_corner_finish_front_m", 1.65))
        if (
            yaw_done > 0.78
            and abs(yaw_error_deg(target_yaw, pose["yaw_deg"])) <= yaw_tol
            and too_close < right < reacquire_m
            and center > finish_front_m
        ):
            self.stop_right_wall_corner(pose, "wall_reacquired")
            return False

        entry_m = float(getattr(self.args, "right_wall_corner_entry_m", 0.55))
        entry_yaw_deg = float(getattr(self.args, "right_wall_corner_entry_yaw_deg", 16.0))
        away_lat = abs(float(getattr(self.args, "right_wall_corner_away_lateral_m_s", 0.14)))
        max_right_lat = abs(float(getattr(self.args, "right_wall_corner_max_right_lateral_m_s", 0.06)))
        hard_front_m = float(getattr(self.args, "right_wall_corner_hard_front_m", 0.95))
        slow_front_m = float(getattr(self.args, "right_wall_corner_slow_front_m", 2.10))

        if travelled < entry_m and center > hard_front_m:
            phase = "entry"
            yaw_step = min(entry_yaw_deg, abs(yaw_error_deg(target_yaw, start_yaw)))
            target_cmd_yaw = wrap_deg(start_yaw + math.copysign(yaw_step, yaw_error_deg(target_yaw, start_yaw)))
            forward = min(
                float(getattr(self.args, "right_wall_corner_entry_speed_m_s", 0.48)),
                float(self.args.cruise_speed_m_s),
            )
            # Deliberately move away from the inside right corner while entering.
            right_cmd = -away_lat if right < float(getattr(self.args, "right_wall_corner_inside_clear_m", 1.35)) or right > reacquire_m else 0.0
        else:
            phase = "arc"
            target_cmd_yaw = target_yaw
            forward = min(
                float(getattr(self.args, "right_wall_corner_arc_speed_m_s", 0.42)),
                float(self.args.cruise_speed_m_s),
            )
            if center < slow_front_m:
                forward = min(forward, float(getattr(self.args, "right_wall_corner_slow_speed_m_s", 0.24)))
            if center < hard_front_m:
                forward = 0.0

            # Keep away from the inside wall through most of the turn.  Only allow
            # a small right drift late in the turn to reacquire the new right wall.
            if right < target_right + float(getattr(self.args, "right_wall_corner_inside_margin_m", 0.25)):
                right_cmd = -away_lat
            elif yaw_done < float(getattr(self.args, "right_wall_corner_allow_right_after_yaw_fraction", 0.65)):
                right_cmd = -0.5 * away_lat
            else:
                clearance_error = right - target_right
                right_cmd = clamp(
                    float(getattr(self.args, "right_wall_lateral_gain", 0.32)) * clearance_error,
                    -away_lat,
                    max_right_lat,
                )

        desired_vn, desired_ve = body_to_ned(forward, right_cmd, float(pose["yaw_deg"]))
        safe_vn, safe_ve, info = self.safety.filter_velocity_ned(desired_vn, desired_ve, pose, depth_frame)

        if info.get("emergency"):
            reason = str(info.get("reason", ""))
            if "front" in reason or center < hard_front_m:
                # Rotate toward the corner exit but do not translate into the wall.
                await self.drone.send_velocity(0.0, 0.0, 0.0, target_cmd_yaw)
                self.record_waypoint(
                    pose,
                    event=f"right_wall_corner_pivot:{phase}",
                    target_yaw=target_cmd_yaw,
                    clearances=(left, center, right),
                    force=True,
                )
                return True
            await self.stop_motion(0.05)
            self.record_waypoint(
                pose,
                event=f"right_wall_corner_emergency_hold:{phase}",
                target_yaw=target_cmd_yaw,
                clearances=(left, center, right),
                force=True,
            )
        ...
        if math.hypot(safe_vn, safe_ve) < float(getattr(self.args, "min_progress_speed_m_s", 0.06)) and forward > 0.0:
            await self.drone.send_velocity(0.0, 0.0, 0.0, target_cmd_yaw)
            self.record_waypoint(
                pose,
                event=f"right_wall_corner_safety_yaw_only:{phase}",
                target_yaw=target_cmd_yaw,
                clearances=(left, center, right),
                force=False,
            )
        ...
        if now - getattr(self, "right_wall_corner_last_log_ms", 0) > 450:
            self.right_wall_corner_last_log_ms = now
            print(
                f"[RIGHT_CORNER] phase={phase} yaw={pose['yaw_deg']:.0f}->{target_cmd_yaw:.0f}/{target_yaw:.0f} "
                f"done={yaw_done:.2f} dist={travelled:.2f} L={left:.2f} C={center:.2f} R={right:.2f} "
                f"cmd_f={forward:.2f} cmd_r={right_cmd:.2f}"
            )

        await self.drone.send_velocity(safe_vn, safe_ve, 0.0, target_cmd_yaw)
        self.right_wall_last_action = f"corner_{phase}"
        self.record_waypoint(
            pose,
            event=f"right_wall_corner_{phase}",
            target_yaw=target_cmd_yaw,
            clearances=(left, center, right),
            force=False,
        )
    ...
    async def drive_path_step_frontier(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray]) -> None:
        left, center, right = self.clearances(depth_frame)
        ...
        # v47: if a committed open-room/gap crossing is active, honour it before
        # ordinary global frontier planning.  This prevents the drone from circling
        # around box clusters when the depth camera says the interior gap is open.
        if await self.drive_open_room_step(pose, depth_frame):
            return
        ...
        # v47: in open rooms, prefer a committed interior/gap crossing toward
        # unknown space over another perimeter loop around boxes.  This is checked
        # after a global path exists so the candidate heading can be compared with
        # the current route, but before the normal segment collision logic rejects
        # narrow-but-flyable gaps.
        if await self.maybe_start_open_room_drive(pose, depth_frame, target, left, center, right, reason="global_path_override"):
            return
        ...
        if bool(getattr(self.args, "avoid_recent_path", True)) and not self.backtrack_targets and not self.recent_escape_active(pose):
        ...
        if not segment_clear:
            # v46: if the same first segment was just verified open by the depth
            # camera, do not enter the GHOST->COMMIT->GHOST loop.  Treat this as a
            # map-only blockage for a short distance and move through it.
            if self.ghost_bypass_active_for(pose, target):
                print("[GHOST] bypass active; ignoring stale map-only blockage on first segment")
                segment_clear = True
            # v35/v46: before rejecting the route and turning back, verify whether
            # the blockage is just a map ghost. If the depth camera confirms the
            # route is open, clear cells and keep moving instead of replanning the
            # identical blocked segment.
            elif await self.verify_and_clear_ghost_segment(pose, target, reason="path_segment_blocked"):
                self.segment_block_count = 0
                self.start_ghost_bypass(pose, target, yaw_from_vector_deg(target[0] - pose["north"], target[1] - pose["east"]))
                segment_clear = True
        ...
        # v45: if we are entering or already inside a corridor-like passage, align
        # the nose more strictly before translating.  The ordinary turn-first
        # threshold is high for speed in rooms, but corridors need better yaw
        # alignment so the depth camera and centreline correction are meaningful.
        if self.corridor_centering_view(left, center, right):
            corridor_align_angle = float(getattr(self.args, "corridor_entry_align_angle_deg", 20.0))
            if yaw_err_abs > corridor_align_angle:
                if now_ms() - getattr(self, "last_corridor_align_log_ms", 0) > 700:
                    self.last_corridor_align_log_ms = now_ms()
                    print(
                        f"[CENTERING] pre-align corridor yaw current={pose['yaw_deg']:.0f} "
        ...
        desired_vn, desired_ve, centering_info = self.apply_global_corridor_centering(
            pose, desired_vn, desired_ve, desired_yaw, left, center, right
        )
    ...
    def _line_blocked_count_for_heading(self, pose: Dict[str, float], yaw_deg: float, distance_m: float, radius_m: float) -> int:
        """Count occupied/inflated map cells in a narrow corridor ahead.

        This uses a deliberately narrower radius than the normal A* inflation so
        a usable gap between two boxes is not rejected simply because the normal
        planning safety radius overlaps both objects.  Reactive depth safety still
        runs before every velocity command.
        """
        yaw = math.radians(float(yaw_deg))
        target = (
            float(pose["north"]) + float(distance_m) * math.cos(yaw),
            float(pose["east"]) + float(distance_m) * math.sin(yaw),
        )
        return len(
            self.mapper.occupied_cells_near_segment(
                (float(pose["north"]), float(pose["east"])),
                target,
                radius_m=float(radius_m),
                ignore_start_m=float(getattr(self.args, "self_clear_path_ignore_start_m", 1.9)),
            )
        )

    def choose_open_room_gateway_heading(self, pose: Dict[str, float], target: Optional[Tuple[float, float]] = None) -> Optional[Dict[str, float]]:
        """Select a non-backtracking, depth/map-plausible direction toward unknown space.

        v47: This is designed for open rooms with box obstacles.  Instead of
        hugging the room perimeter or circling around a cluster of boxes, score
        straight-through interior/gap headings by unknown-space gain and recent
        path avoidance.  It does not command motion directly; it only starts a
        committed local route if the score is clearly useful.
        """
        if not bool(getattr(self.args, "open_room_gateway_mode", True)):
            return None
        if self.backtrack_targets:
            return None
        if now_ms() - self.open_room_last_select_ms < int(1000.0 * float(getattr(self.args, "open_room_select_gap_s", 2.5))):
            return None

        base = float(pose.get("yaw_deg", 0.0))
        candidate_yaws: List[float] = []
        if target is not None:
            candidate_yaws.append(yaw_from_vector_deg(float(target[0]) - float(pose["north"]), float(target[1]) - float(pose["east"])))
        candidate_yaws.extend([base, base + 30.0, base - 30.0, base + 60.0, base - 60.0, base + 90.0, base - 90.0])

        # Deduplicate by yaw bin while preserving order.
        unique: List[float] = []
        seen: Set[int] = set()
        for y in candidate_yaws:
            wy = wrap_deg(y)
            k = int(round(wy / 10.0))
            if k not in seen:
                seen.add(k)
        ...
        lookahead = float(getattr(self.args, "open_room_gateway_lookahead_m", 7.5))
        width = float(getattr(self.args, "open_room_gateway_width_m", 2.2))
        radius = float(getattr(self.args, "open_room_gap_path_radius_m", 0.28))
        max_blocked = int(getattr(self.args, "open_room_max_blocked_cells", 4))
        min_unknown = int(getattr(self.args, "open_room_min_unknown_cells", 28))

        best: Optional[Dict[str, float]] = None
        for yaw in unique:
            yaw_err = abs(yaw_error_deg(yaw, base))
            if yaw_err > float(getattr(self.args, "open_room_max_turn_deg", 115.0)):
                continue
            if self.heading_points_to_dead_end_memory(pose, yaw, lookahead_m=min(lookahead, 5.0)):
                continue

            blocked = self._line_blocked_count_for_heading(pose, yaw, min(lookahead, 5.0), radius)
            if blocked > max_blocked:
                continue

            unknown = self._unknown_gain_ahead(pose, yaw, lookahead, width_m=width)
            if unknown < min_unknown:
                continue

            visited = self._visited_penalty_ahead(pose, yaw, min(lookahead, 5.0))
            recent = self.heading_points_into_recent_path(pose, yaw, min(lookahead, float(getattr(self.args, "recent_heading_lookahead_m", 4.0))))
            recent_penalty = float(getattr(self.args, "open_room_recent_penalty", 28.0)) if recent else 0.0
            turn_penalty = float(getattr(self.args, "open_room_turn_penalty", 0.035)) * yaw_err
            blocked_penalty = float(getattr(self.args, "open_room_blocked_cell_penalty", 1.2)) * float(blocked)
            score = (
                float(getattr(self.args, "open_room_unknown_weight", 0.55)) * float(unknown)
                - float(getattr(self.args, "open_room_visited_penalty", 2.0)) * float(visited)
                - recent_penalty
                - turn_penalty
                - blocked_penalty
            )
        ...
        if best is None:
            return None
        if best["score"] < float(getattr(self.args, "open_room_min_score", 10.0)):
            return None
        return best

    def start_open_room_drive(self, pose: Dict[str, float], yaw_deg: float, reason: str, score: float = 0.0) -> None:
        self.open_room_drive_yaw = wrap_deg(float(yaw_deg))
        self.open_room_drive_until_ms = now_ms() + int(1000.0 * float(getattr(self.args, "open_room_commit_s", 9.0)))
        self.open_room_drive_start_pose = dict(pose)
        self.open_room_last_select_ms = now_ms()
        self.current_path = []
        self.local_corridor_yaw = None
        self.stop_express_corridor(pose, reason="open_room_gateway")
        self.start_progress_commitment(
            pose,
            self.open_room_drive_yaw,
            f"open_room:{reason}",
            duration_s=float(getattr(self.args, "open_room_commit_s", 9.0)),
            min_dist_m=float(getattr(self.args, "open_room_min_commit_m", 4.0)),
        )
        print(f"[OPEN_ROOM] selected yaw={self.open_room_drive_yaw:.0f} reason={reason} score={score:.1f}")
        self.record_waypoint(pose, event=f"open_room_selected:{reason}", target_yaw=self.open_room_drive_yaw, force=True)

    async def drive_open_room_step(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray]) -> bool:
        """Drive a committed open-room/interior-gap heading for one control step."""
        if self.open_room_drive_yaw is None:
            return False
        if now_ms() > self.open_room_drive_until_ms:
            self.open_room_drive_yaw = None
            return False
        if self.open_room_drive_start_pose is not None:
            travelled = math.hypot(
                float(pose["north"]) - float(self.open_room_drive_start_pose["north"]),
                float(pose["east"]) - float(self.open_room_drive_start_pose["east"]),
            )
            if travelled >= float(getattr(self.args, "open_room_max_commit_m", 9.0)):
                self.open_room_drive_yaw = None
                return False

        left, center, right = self.clearances(depth_frame)
        # If there is a real front obstacle, immediately return to the existing
        # front-block/dead-end logic instead of pushing through.
        if center < float(getattr(self.args, "open_room_abort_center_m", 1.35)):
            supported = False if depth_frame is None else bool(self.safety.front_obstacle_supported(depth_frame, min(float(getattr(self.args, "emergency_stop_m", 1.05)) + 0.30, center + 0.05)))
            if supported:
                print(f"[OPEN_ROOM] abort: supported front obstacle C={center:.2f}")
                self.open_room_drive_yaw = None
                return False

        target_yaw = float(self.open_room_drive_yaw)
        yaw_err = yaw_error_deg(target_yaw, float(pose["yaw_deg"]))
        if abs(yaw_err) > float(getattr(self.args, "open_room_yaw_tolerance_deg", 30.0)):
            await self.drone.send_velocity(0.0, 0.0, 0.0, target_yaw)
            return True

        speed = min(float(getattr(self.args, "open_room_speed_m_s", self.args.cruise_speed_m_s)), float(self.args.cruise_speed_m_s))
        if center < float(getattr(self.args, "front_slow_m", 2.8)):
            speed = min(speed, float(self.args.slow_speed_m_s))

        # Keep only gentle lateral centering in a cluttered room.  Large centering
        # corrections near boxes can cause a perimeter orbit instead of a gap crossing.
        right_cmd = 0.0
        if min(left, right) < float(getattr(self.args, "open_room_side_balance_trigger_m", 1.35)) and max(left, right) < float(getattr(self.args, "open_room_side_balance_open_max_m", 5.5)):
            balance_error = clamp((right - left) / max(1.0, left + right), -1.0, 1.0)
            right_cmd = clamp(
                float(getattr(self.args, "open_room_lateral_gain", 0.12)) * balance_error,
                -float(getattr(self.args, "open_room_max_lateral_m_s", 0.12)),
                float(getattr(self.args, "open_room_max_lateral_m_s", 0.12)),
            )

        desired_vn, desired_ve = body_to_ned(speed, right_cmd, float(pose["yaw_deg"]))
        safe_vn, safe_ve, info = self.safety.filter_velocity_ned(desired_vn, desired_ve, pose, depth_frame)
        if info.get("active") and now_ms() - getattr(self, "last_open_room_avoid_log_ms", 0) > 600:
            self.last_open_room_avoid_log_ms = now_ms()
            print(
                f"[OPEN_ROOM_AVOID] {info.get('reason')} desired=({desired_vn:.2f},{desired_ve:.2f}) "
                f"safe=({safe_vn:.2f},{safe_ve:.2f}) L={info.get('left',0):.2f} C={info.get('center',0):.2f} R={info.get('right',0):.2f}"
            )
        if info.get("emergency"):
            self.open_room_drive_yaw = None
            return False
        if math.hypot(safe_vn, safe_ve) < float(getattr(self.args, "min_progress_speed_m_s", 0.06)):
            # Keep a tiny crawl only if the camera center is open; otherwise release
            # to the normal planner/recovery logic.
            if center > float(getattr(self.args, "corridor_clear_front_override_m", 3.0)):
                safe_vn, safe_ve = body_to_ned(float(getattr(self.args, "corridor_crawl_speed_m_s", 0.12)), 0.0, float(pose["yaw_deg"]))
            else:
                self.open_room_drive_yaw = None
                return False
        await self.drone.send_velocity(safe_vn, safe_ve, 0.0, target_yaw)
        self.record_waypoint(pose, event="open_room_move", target_yaw=target_yaw, clearances=(left, center, right))
        return True

    async def maybe_start_open_room_drive(self, pose: Dict[str, float], depth_frame: Optional[np.ndarray], target: Optional[Tuple[float, float]], left: float, center: float, right: float, reason: str) -> bool:
        """Start a committed open-room/gap crossing if it is clearly beneficial."""
        if not bool(getattr(self.args, "open_room_gateway_mode", True)):
            return False
        if self.progress_commit_active(pose) and not bool(getattr(self.args, "open_room_can_override_global_commit", True)):
            return False
        if center < float(getattr(self.args, "open_room_min_center_m", 3.0)):
            return False
        if min(left, right) < float(getattr(self.args, "open_room_min_side_m", 0.75)):
            return False
        # Trigger mainly in room-like views or when the current route has already
        # been circling/using a lot of recent cells.  This prevents constant route
        # switching in narrow corridors while allowing gap crossing between boxes.
        room_like = (max(left, right) > float(getattr(self.args, "open_room_roomlike_side_m", 3.2))) or (center > float(getattr(self.args, "open_room_roomlike_center_m", 5.0)))
        if not room_like:
            return False
        best = self.choose_open_room_gateway_heading(pose, target=target)
        if best is None:
            return False
        self.start_open_room_drive(pose, float(best["yaw"]), reason=reason, score=float(best["score"]))
        return await self.drive_open_room_step(pose, depth_frame)
    ...
    async def run(self) -> None:
        ...
        print(f"[MISSION] navigation_mode={getattr(self.args, 'navigation_mode', 'right_wall')}")
        ...
        if str(getattr(self.args, "navigation_mode", "right_wall")).lower() == "right_wall":
            startup_mode = "none"
            print("[MISSION] right-wall navigation active; startup frontier/centreline scans skipped")
        elif bool(getattr(self.args, "no_startup_scan", False)):
            startup_mode = "none"
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Right-wall-following qualifier mission with unchanged YOLO barrel counting, occupancy grid mapping, and self-footprint cleanup. The original v49 frontier navigator is still available with --navigation-mode frontier.")

    p.add_argument("--mode", choices=["collect", "count"], default="count")
    p.add_argument("--model", default="models/barrel_best.pt", help="YOLO model path for count mode. Default is baked in so this file can be run directly with python3 qualifier_right_wall_v51.py.")
    ...
    p.add_argument("--output-dir", default="qualifier_right_wall_v51_output")
    ...
    p.add_argument("--takeoff-altitude-m", type=float, default=3.2, help="Target takeoff altitude in metres above the start point. Default baked in for direct v51 runs.")
    ...
    p.add_argument("--no-land", action="store_true", default=True, help="Leave drone hovering/offboard at the end by default for v51 direct runs")

    # Navigation mode.  right_wall is the default for this file; frontier keeps
    # the original v49 planner available for comparison.
    p.add_argument("--navigation-mode", choices=["right_wall", "frontier"], default="right_wall", help="Navigation policy: right_wall follows the right wall only; frontier runs the original v49 planner")
    p.add_argument("--right-wall-target-m", type=float, default=1.30, help="Desired clearance from the right wall")
    p.add_argument("--right-wall-open-m", type=float, default=2.40, help="Right sector clearance treated as an opening/right turn")
    p.add_argument("--right-wall-open-confirm-frames", type=int, default=2, help="Consecutive frames before committing to a right opening")
    p.add_argument("--right-wall-front-turn-m", type=float, default=1.55, help="Front clearance below this triggers a left turn unless the right side is open")
    p.add_argument("--right-wall-corner-min-front-m", type=float, default=0.85, help="Minimum front clearance for turning into a right opening")
    p.add_argument("--right-wall-too-close-m", type=float, default=0.70, help="Right clearance below this forces a left lateral correction")
    p.add_argument("--right-wall-speed-m-s", type=float, default=0.95, help="Nominal right-wall-following forward speed")
    p.add_argument("--right-wall-corner-forward-m-s", type=float, default=0.30, help="Legacy small forward speed while turning into a right opening; staged corner mode uses the entry/arc speed flags below")
    p.add_argument("--right-wall-close-speed-m-s", type=float, default=0.36, help="Forward speed when too close to the right wall")
    p.add_argument("--right-wall-slow-front-m", type=float, default=2.40, help="Start using slow speed when front clearance is below this")
    p.add_argument("--right-wall-lateral-gain", type=float, default=0.32, help="Body-right velocity gain for right-wall clearance error")
    p.add_argument("--right-wall-max-lateral-m-s", type=float, default=0.24, help="Maximum body-right correction speed")
    p.add_argument("--right-wall-search-lateral-m-s", type=float, default=0.12, help="Gentle right drift while searching for a right wall")
    p.add_argument("--right-wall-yaw-gain-deg-per-m", type=float, default=10.0, help="Yaw bias gain toward/away from the right wall")
    p.add_argument("--right-wall-max-yaw-bias-deg", type=float, default=14.0, help="Maximum continuous yaw bias while following a wall")
    p.add_argument("--right-wall-right-turn-deg", type=float, default=32.0, help="Yaw step used when a right opening appears")
    p.add_argument("--right-wall-left-turn-deg", type=float, default=36.0, help="Yaw step used when the front is blocked")
    p.add_argument("--right-wall-log-interval-s", type=float, default=0.5, help="Minimum interval between right-wall status logs")
    p.add_argument("--right-wall-corner-turn-deg", type=float, default=90.0, help="Total yaw change for a detected right corner")
    p.add_argument("--right-wall-corner-entry-m", type=float, default=0.55, help="Forward distance to carry past the inside corner before fully arcing right")
    p.add_argument("--right-wall-corner-entry-yaw-deg", type=float, default=16.0, help="Small initial yaw during the wide-entry phase")
    p.add_argument("--right-wall-corner-entry-speed-m-s", type=float, default=0.48, help="Forward speed during the wide-entry phase")
    p.add_argument("--right-wall-corner-arc-speed-m-s", type=float, default=0.42, help="Forward speed while arcing around a right corner")
    p.add_argument("--right-wall-corner-slow-speed-m-s", type=float, default=0.24, help="Corner arc speed when the front clearance is reduced")
    p.add_argument("--right-wall-corner-away-lateral-m-s", type=float, default=0.14, help="Body-left bias used to avoid clipping the inside right corner")
    p.add_argument("--right-wall-corner-max-right-lateral-m-s", type=float, default=0.06, help="Maximum body-right drift allowed late in a right-corner arc")
    p.add_argument("--right-wall-corner-inside-clear-m", type=float, default=1.35, help="Clearance threshold used to bias away from the inside corner during entry")
    p.add_argument("--right-wall-corner-inside-margin-m", type=float, default=0.25, help="Extra margin above target right clearance during a corner")
    p.add_argument("--right-wall-corner-slow-front-m", type=float, default=2.10, help="Front clearance below this slows a right-corner arc")
    p.add_argument("--right-wall-corner-hard-front-m", type=float, default=0.95, help="Front clearance below this pivots instead of translating during a right-corner arc")
    p.add_argument("--right-wall-corner-reacquire-m", type=float, default=2.05, help="Right clearance below this means the new right wall has been reacquired")
    p.add_argument("--right-wall-corner-finish-yaw-tol-deg", type=float, default=16.0, help="Yaw tolerance for finishing a right-corner arc")
    p.add_argument("--right-wall-corner-finish-front-m", type=float, default=1.65, help="Minimum front clearance required before finishing a right-corner arc")
    p.add_argument("--right-wall-corner-allow-right-after-yaw-fraction", type=float, default=0.65, help="Only allow small right drift after this fraction of corner yaw is complete")
    p.add_argument("--right-wall-corner-timeout-s", type=float, default=5.5, help="Timeout for one staged right-corner manoeuvre")
    ...
    p.add_argument("--topo-dead-end-scan-max-headings", type=int, default=6, help="v48: headings to test when escaping a confirmed dead end; checks U-turn/recent corridor first")
    p.add_argument("--topo-dead-end-escape-min-center-m", type=float, default=1.75, help="v48: minimum centre clearance for a permissive dead-end escape heading")
    p.add_argument("--topo-dead-end-escape-min-side-m", type=float, default=0.35, help="v48: minimum side clearance for a permissive dead-end escape heading")
    ...
    p.add_argument("--escape-frontier-fallback", action=argparse.BooleanOptionalAction, default=True, help="v48: after strict non-backtracking planning fails, allow a short reuse of recent cells only if it reaches a new unexplored frontier")
    p.add_argument("--escape-recent-path-penalty", type=float, default=2.0, help="Penalty for recent cells during the escape fallback planner; lower than normal to permit leaving local loops")
    p.add_argument("--recent-escape-commit-min-m", type=float, default=5.8, help="v49: once an escape fallback route intentionally reuses recent cells, follow it for this distance before applying no-backtracking again")
    p.add_argument("--recent-escape-commit-s", type=float, default=16.0, help="v49: time limit for the recent-path escape lock")
    ...
    p.add_argument("--ghost-clear-radius-m", type=float, default=1.25)
    p.add_argument("--ghost-clear-amount", type=int, default=12)
    ...
    p.add_argument("--ghost-bypass-after-clear-s", type=float, default=5.0, help="After a map blockage is depth-verified open, ignore that map-only blockage briefly instead of replanning into the same loop")
    p.add_argument("--ghost-bypass-min-m", type=float, default=2.8, help="Minimum distance allowed through a depth-verified ghost blockage before normal map checks resume")
    p.add_argument("--ghost-bypass-target-tolerance-m", type=float, default=2.2)
    p.add_argument("--ghost-bypass-yaw-tolerance-deg", type=float, default=35.0)
    p.add_argument("--dead-end-escape-gap-s", type=float, default=2.0)
    p.add_argument("--dead-end-escape-on-front-block-fail", action=argparse.BooleanOptionalAction, default=True, help="v48: if no fresh side branch is found from a front block, immediately choose a permissive escape/backtrack heading")
    p.add_argument("--dead-end-force-breadcrumb-after-failed-bypass", action=argparse.BooleanOptionalAction, default=True, help="v48: after front-block side scans fail, breadcrumb backtrack is allowed even if raw side sectors look open")
    p.add_argument("--dead-end-scan-max-headings", type=int, default=6, help="v48: local heading budget for dead-end/backtrack scans")
    ...
    p.add_argument("--zone-loop-min-travel-m", type=float, default=22.0)
    p.add_argument("--zone-loop-max-span-m", type=float, default=12.0)
    p.add_argument("--zone-loop-close-m", type=float, default=2.0)
    p.add_argument("--zone-loop-min-current-visit", type=int, default=8)
    p.add_argument("--zone-loop-cooldown-s", type=float, default=14.0)
    ...
    p.add_argument("--global-corridor-centering", action=argparse.BooleanOptionalAction, default=True, help="v45: while following a global route through a corridor, bias the command toward the local centreline instead of hugging/clipping walls")
    p.add_argument("--corridor-entry-align-angle-deg", type=float, default=20.0, help="v45: in corridor-like passages, align yaw this closely before translating")
    p.add_argument("--corridor-centering-min-center-m", type=float, default=2.4, help="Minimum forward clearance before corridor-centering can activate")
    p.add_argument("--corridor-centering-min-valid-side-m", type=float, default=0.25)
    p.add_argument("--corridor-centering-side-wall-max-m", type=float, default=3.8, help="Side clearances below this on both sides count as corridor walls")
    p.add_argument("--corridor-centering-open-side-max-m", type=float, default=6.0, help="Maximum opposite-side clearance when only one wall is close")
    p.add_argument("--corridor-centering-one-side-trigger-m", type=float, default=1.65, help="Activate centering if one side is this close and the forward path is open")
    p.add_argument("--corridor-centering-balance-deadband-m", type=float, default=0.20)
    p.add_argument("--corridor-centering-gain", type=float, default=0.22)
    p.add_argument("--corridor-centering-max-lateral-m-s", type=float, default=0.22)
    p.add_argument("--corridor-centering-close-side-m", type=float, default=0.85)
    p.add_argument("--corridor-centering-slow-imbalance-m", type=float, default=1.15)
    p.add_argument("--corridor-centering-entry-speed-m-s", type=float, default=0.55)
    p.add_argument("--corridor-centering-close-speed-m-s", type=float, default=0.42)
    p.add_argument("--open-room-gateway-mode", action=argparse.BooleanOptionalAction, default=True, help="v47: in large rooms, commit through depth-clear interior gaps toward unknown space instead of circling box clusters/perimeters")
    p.add_argument("--open-room-select-gap-s", type=float, default=2.5)
    p.add_argument("--open-room-min-center-m", type=float, default=3.0)
    p.add_argument("--open-room-min-side-m", type=float, default=0.75)
    p.add_argument("--open-room-roomlike-side-m", type=float, default=3.2)
    p.add_argument("--open-room-roomlike-center-m", type=float, default=5.0)
    p.add_argument("--open-room-gateway-lookahead-m", type=float, default=7.5)
    p.add_argument("--open-room-gateway-width-m", type=float, default=2.2)
    p.add_argument("--open-room-gap-path-radius-m", type=float, default=0.28)
    p.add_argument("--open-room-max-blocked-cells", type=int, default=4)
    p.add_argument("--open-room-min-unknown-cells", type=int, default=28)
    p.add_argument("--open-room-min-score", type=float, default=10.0)
    p.add_argument("--open-room-unknown-weight", type=float, default=0.55)
    p.add_argument("--open-room-visited-penalty", type=float, default=2.0)
    p.add_argument("--open-room-recent-penalty", type=float, default=28.0)
    p.add_argument("--open-room-turn-penalty", type=float, default=0.035)
    p.add_argument("--open-room-blocked-cell-penalty", type=float, default=1.2)
    p.add_argument("--open-room-max-turn-deg", type=float, default=115.0)
    p.add_argument("--open-room-commit-s", type=float, default=9.0)
    p.add_argument("--open-room-min-commit-m", type=float, default=4.0)
    p.add_argument("--open-room-max-commit-m", type=float, default=9.0)
    p.add_argument("--open-room-speed-m-s", type=float, default=1.05)
    p.add_argument("--open-room-yaw-tolerance-deg", type=float, default=30.0)
    p.add_argument("--open-room-abort-center-m", type=float, default=1.35)
    p.add_argument("--open-room-side-balance-trigger-m", type=float, default=1.35)
    p.add_argument("--open-room-side-balance-open-max-m", type=float, default=5.5)
    p.add_argument("--open-room-lateral-gain", type=float, default=0.12)
    p.add_argument("--open-room-max-lateral-m-s", type=float, default=0.12)
    p.add_argument("--open-room-can-override-global-commit", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--progress-commit-min-m", type=float, default=6.0, help="Distance to travel before non-essential gateway/zone-loop rescans are allowed again")
    p.add_argument("--progress-commit-s", type=float, default=12.0, help="Maximum time for a progress commitment before rescans are allowed again")
